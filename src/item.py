...
class _Item():
    ...
    def f_get_item_lm(self, review_corpus, stop_words):
        ...
        self.m_avg_len = total_doc_len/D
        
        for word in self.m_word_tf_map:
            word_tf = self.m_word_tf_map[word]

            word_lm = word_tf/total_doc_len
            
            self.m_avg_review_words[word] = word_lm

    def f_get_RRe(self, review_obj, stop_words, k1 = 1.5, b = 0.75):
        # for review_id in self.m_review_id_list:
        #     review_obj = review_corpus[review_id]
        ...
        for word in word_tf_review_map:
            if word in stop_words:
                print("error in tf words")
                continue

            if word not in self.m_word_df_map:
                continue

            word_tf = word_tf_review_map[word]
